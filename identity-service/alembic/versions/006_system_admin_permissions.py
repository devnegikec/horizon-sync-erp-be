...
def upgrade():
    ...
    # Ensure required enum values exist before inserting permissions
    op.execute(
        ...
        "'all', 'report'); "
        "ELSE "
        "ALTER TYPE resourcetype ADD VALUE IF NOT EXISTS 'all'; "
        "ALTER TYPE resourcetype ADD VALUE IF NOT EXISTS 'report'; "
        "END IF; END $$;"
    )
...
def downgrade():
    ...
    for code in permission_codes:
        session.execute(
            text("DELETE FROM permissions WHERE code = :code"), {"code": code}
        )
