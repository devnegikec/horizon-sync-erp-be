...
class SystemAdminPermissionService:
    ...
    def assign_user_as_system_admin(
        ...
        if existing_role:
            logger.info(
                f"User {user_id} already has system admin role in org {master_organization_id}"
            )
        ...
        logger.info(
            f"Assigned user {user_id} as {admin_type} in master org {master_organization_id}"
        )
        return user_role
    ...
    def get_system_admin_permissions(self, user_id: UUID) -> list[str]:
        ...
        for user_role in user_roles:
            ...
            if role:
                ...
                role_permissions = self.role_service.get_role_permissions(
                    user_role.role_id
                )
    ...
    def can_access_organization(
        self, user_id: UUID, target_organization_id: UUID
    ) -> bool:
    ...
    def assign_customer_organization_to_master(
        ...
        logger.info(
            f"Linked customer org {customer_organization_id} to master org {master_organization_id}"
        )
        return customer_org
    ...
    def _get_or_create_system_admin_role(self, admin_type: str) -> Role:
        ...
        role = self.db.query(Role).filter(Role.name == admin_type).first()
    ...
    def get_system_admin_users(
        ...
        return {"users": enhanced_users, "total": total}
    ...
    def get_system_admin_audit_logs(
        ...
        if target_organization_id:
            query = query.filter(
                SystemAdminAuditLog.target_organization_id == target_organization_id
            )
        ...
        offset = (page - 1) * page_size
        audit_logs = (
            query.order_by(SystemAdminAuditLog.performed_date.desc())
            .offset(offset)
            .limit(page_size)
            .all()
        )
    ...
    def get_audit_log_stats(self) -> dict:
        ...
        type_counts = (
            self.db.query(
                SystemAdminAuditLog.action_type, func.count(SystemAdminAuditLog.id)
            )
        ...
        admin_counts = (
            self.db.query(
                SystemAdminAuditLog.admin_username, func.count(SystemAdminAuditLog.id)
            )
