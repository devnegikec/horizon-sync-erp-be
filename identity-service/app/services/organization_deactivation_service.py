...
from app.models.organization import BillingStatus, Organization, OrganizationType
...
class OrganizationDeactivationService:
    ...
    def check_organizations_for_deactivation(self) -> list[dict]:
            ...
            actions_needed.append(
                {
                    ...
                    "days_expired": (current_date - org.trial_end_date).days
        ...
        subscription_expired = self._get_subscription_expired_organizations(
            current_date
        )
            ...
            actions_needed.append(
                {
                    ...
                    "days_expired": (current_date - org.subscription_end_date).days
        ...
        for org_data in overdue_orgs:
            ...
            if days_overdue >= 90:
                ...
                reason = (
                    f"Payment overdue for {days_overdue} days - suspension required"
                )
    ...
    def expire_trial_organization(
        self, organization_id: UUID, expired_by: Optional[UUID] = None
    ) -> dict:
        ...
        logger.info(
            f"Expired trial for organization {organization_id} by {expired_by or 'system'}"
        )
    ...
    def expire_subscription_organization(
        self, organization_id: UUID, expired_by: Optional[UUID] = None
    ) -> dict:
        ...
        logger.info(
            f"Expired subscription for organization {organization_id} by {expired_by or 'system'}"
        )
    ...
    def suspend_organization_for_nonpayment(
        ...
        self._send_deactivation_notification(
            org,
            DeactivationType.NONPAYMENT_SUSPENSION,
            suspended_by,
            additional_context={"days_overdue": days_overdue},
        )

        logger.info(
            f"Suspended organization {organization_id} for {days_overdue} days overdue by {suspended_by or 'system'}"
        )
    ...
    def cancel_organization_subscription(
        ...
        self._send_deactivation_notification(
            org,
            DeactivationType.MANUAL_CANCELLATION,
            cancelled_by,
            additional_context={
        ...
        logger.info(
            f"Cancelled subscription for organization {organization_id}: {cancellation_reason}"
        )
    ...
    def reactivate_organization(
        ...
        logger.info(
            f"Reactivated organization {organization_id} until {new_subscription_end_date.date()} by {reactivated_by}"
        )
    ...
    def get_deactivation_summary(self) -> dict:
        ...
        return {
            ...
            "subscription_expired_count": status_counts.get(
                BillingStatus.EXPIRED.value, 0
            ),
    ...
    def get_organizations_requiring_action(self) -> dict:
        ...
        for org in trial_expired:
            days_expired = (
                (current_date - org.trial_end_date).days if org.trial_end_date else 0
            )
        ...
        for org in subscription_expired:
            days_expired = (
                (current_date - org.subscription_end_date).days
                if org.subscription_end_date
                else 0
            )
    ...
    def _get_trial_expired_organizations(
    ...
    def _get_subscription_expired_organizations(
        self, current_date: datetime.date
    ) -> list[Organization]:
        ...
        return (
            self.db.query(Organization)
            .filter(
                Organization.billing_status.in_(
                    [BillingStatus.ACTIVE, BillingStatus.OVERDUE]
                ),
                Organization.subscription_end_date < current_date,
                Organization.organization_type == OrganizationType.CUSTOMER,
            )
    ...
    def _get_overdue_payment_organizations(
        self, current_date: datetime.date
    ) -> list[dict]:
        ...
        for org in overdue_orgs:
            if org.next_billing_date:
                ...
                result.append({"organization": org, "days_overdue": days_overdue})
    ...
    def _send_deactivation_notification(
        ...
        try:
            ...
            logger.info(
                f"Sent deactivation notification for org {organization.id} type {deactivation_type}"
            )
