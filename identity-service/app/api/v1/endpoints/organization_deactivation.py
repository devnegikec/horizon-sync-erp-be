...
class ExpireTrialRequest(BaseModel):
    ...
    reason: Optional[str] = Field(
        "Trial period expired", description="Reason for trial expiration"
    )
    send_notification: bool = Field(
        True, description="Send expiration notification email"
    )
...
class ExpireSubscriptionRequest(BaseModel):
    ...
    reason: Optional[str] = Field(
        "Subscription expired", description="Reason for subscription expiration"
    )
    send_notification: bool = Field(
        True, description="Send expiration notification email"
    )
...
class SuspendOrganizationRequest(BaseModel):
    ...
    days_overdue: int = Field(
        ..., ge=1, description="Number of days payment is overdue"
    )
    suspension_reason: Optional[str] = Field(
        None, description="Additional suspension reason"
    )
    send_notification: bool = Field(
        True, description="Send suspension notification email"
    )
...
class CancelSubscriptionRequest(BaseModel):
    ...
    grace_period_days: int = Field(
        30, ge=0, le=90, description="Grace period before deactivation"
    )
    send_notification: bool = Field(
        True, description="Send cancellation notification email"
    )
...
class ReactivateOrganizationRequest(BaseModel):
    ...
    new_subscription_end_date: datetime = Field(
        ..., description="New subscription end date"
    )
    reactivation_notes: Optional[str] = Field(
        None, description="Notes about reactivation"
    )
    send_notification: bool = Field(
        True, description="Send reactivation notification email"
    )
...
async def check_organizations_for_deactivation(
    ...
    try:
        ...
        logger.info(
            f"Found {len(actions_needed)} organizations requiring deactivation action"
        )
...
@router.get(
    ...
    response_model=OrganizationsRequiringActionResponse,
)
...
@router.post(
    "/expire-trial/{organization_id}", response_model=DeactivationActionResponse
)
async def expire_trial(
    ...
    try:
        ...
        result = service.expire_trial_organization(
            organization_id=organization_id, expired_by=current_user.id
        )

        logger.info(
            f"Expired trial for organization {organization_id} by user {current_user.id}"
        )
...
@router.post(
    "/expire-subscription/{organization_id}", response_model=DeactivationActionResponse
)
async def expire_subscription(
    ...
    try:
        ...
        logger.info(
            f"Expired subscription for organization {organization_id} by user {current_user.id}"
        )
        ...
        logger.error(
            f"Failed to expire subscription for organization {organization_id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def suspend_organization(
    ...
    try:
        ...
        logger.info(
            f"Suspended organization {organization_id} for {request.days_overdue} days overdue"
        )
        ...
        return DeactivationActionResponse(
            ...
            or f"Payment overdue for {request.days_overdue} days",
            additional_info={
...
async def cancel_subscription(
    ...
    try:
        ...
        logger.info(
            f"Cancelled subscription for organization {organization_id}: {request.cancellation_reason}"
        )
        ...
        logger.error(
            f"Failed to cancel subscription for organization {organization_id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def reactivate_organization(
    ...
    try:
        ...
        logger.info(
            f"Reactivated organization {organization_id} until {request.new_subscription_end_date}"
        )
...
async def bulk_suspend_organizations(
    ...
    days_overdue: int = Query(
        ..., ge=1, description="Days overdue for all organizations"
    ),
    ...
    try:
        ...
        for org_id in organization_ids:
            try:
                ...
                results.append(
                    {"organization_id": org_id, "status": "failed", "error": str(e)}
                )
...
@router.get(
    "/organization-status/{organization_id}", response_model=OrganizationStatusResponse
)
    ...
    try:
        ...
        if org.billing_status == BillingStatus.TRIAL and org.trial_end_date:
            ...
            if days_overdue >= 90:
                ...
                risk_level = "high"
                risk_reason = (
                    f"Payment overdue for {days_overdue} days - final notice required"
                )
            elif days_overdue >= 30:
                risk_level = "medium"
                risk_reason = (
                    f"Payment overdue for {days_overdue} days - reminder required"
                )
        ...
        return {
            ...
            "deactivation_risk": {"level": risk_level, "reason": risk_reason},
            "status_history": {
                "customer_since": org.customer_since,
                "current_status_since": org.subscription_start_date
                or org.customer_since,
            },
        }
