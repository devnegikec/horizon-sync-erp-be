...
def create_draft_payments(db: Session, org_id: uuid.UUID, admin_user_id: uuid.UUID):
    ...
    for i in range(min(5, len(customer_invoices))):
        try:
            ...
            draft_count += 1
            print(
                f"  Created draft payment {payment.id} - ${payment.amount} ({payment_mode.value})"
            )
...
def create_draft_payments_with_allocations(
    ...
    for i in range(5, min(10, len(customer_invoices))):
        try:
            invoice = customer_invoices[i]
            payment_mode = (
                PaymentMode.BANK_TRANSFER if i % 2 == 0 else PaymentMode.CHECK
            )
            ...
            print(
                f"  Created draft payment {payment.id} with allocation - ${payment.amount}"
            )
...
def create_confirmed_payments(db: Session, org_id: uuid.UUID, admin_user_id: uuid.UUID):
    ...
    for i in range(10, min(15, len(customer_invoices))):
        try:
            ...
            confirmed_count += 1
            print(
                f"  Created confirmed payment {confirmed_payment.receipt_number} - ${payment.amount}"
            )
...
def create_confirmed_payments_with_unallocated(
    db: Session, org_id: uuid.UUID, admin_user_id: uuid.UUID
):
    ...
    for i in range(15, min(18, len(customer_invoices))):
        try:
            ...
            print(
                f"  Created payment {confirmed_payment.receipt_number} - ${payment_amount} (${unallocated_amt} unallocated)"
            )
...
def create_supplier_payments(db: Session, org_id: uuid.UUID, admin_user_id: uuid.UUID):
    ...
    for i in range(3, min(8, len(supplier_invoices))):
        try:
            ...
            supplier_payment_count += 1
            print(
                f"  Created confirmed supplier payment {confirmed_payment.receipt_number} - ${payment.amount}"
            )
...
def main():
    ...
    try:
        ...
        allocated_draft_count = create_draft_payments_with_allocations(
        ...
        unallocated_count = create_confirmed_payments_with_unallocated(
            db, ORG_ID, ADMIN_USER_ID
        )
        db.commit()
