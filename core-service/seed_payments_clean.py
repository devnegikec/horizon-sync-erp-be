"""Seed Payment Flow data for testing - Task 21.4

This script creates cancelled payment scenarios.

Usage:
    python seed_payments_clean.py
"""
...
def create_cancelled_payments(db: Session, org_id: uuid.UUID, admin_user_id: uuid.UUID):
    ...
    # Get unpaid customer invoices - using actual column names from database
    ...
    customer_invoices = db.execute(
        customer_invoices_query, {"org_id": org_id}
    ).fetchall()

    if len(customer_invoices) < 5:
        print(f"  Warning: Only {len(customer_invoices)} customer invoices available.")
        print("  Need to seed invoices first. Skipping cancelled payments.")
        return

    # Get unpaid supplier invoices - using actual column names from database
    ...
    supplier_invoices = db.execute(
        supplier_invoices_query, {"org_id": org_id}
    ).fetchall()
