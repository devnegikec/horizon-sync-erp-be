...
| ID | Function | Status | Est. (days) | Notes |
|---|---|---|---|---|
| WF-001 | Receive SAP SO/Delivery | 🟡 | 0.5 | `create_from_invoice` + `/import` exist; no duplicate/malformed rejection + interface log retention |
| WF-002 | Validate order eligibility (master data exists) | ❌ | 0.5 | No SKU/UoM/location-master existence check → exception hold |
| WF-003 | Check ATP (exclude blocked/damaged/hold/quality) | 🟡 | 1 | FEFO/FIFO resolution exists, but only filters `quantity_on_hand > 0` — no status exclusion; no configurable backorder/partial rule |
| WF-004 | Allocate / reserve inventory | 🟡 | 1 | Worker bin-reservation exists; no demand-level reservation + partial-allocation policy |
| WF-005 | Pick strategy (FIFO/FEFO, fixed-bin, bulk-break, zone, bin-priority) | 🟡 | 2.5 | FEFO/FIFO done; fixed-bin/zone/bulk-break/bin-priority not implemented |
| WF-006 | Create pick task/list | ✅ | 0.5 | `create_from_invoice`, `create`, task creation via `TaskService` |
| WF-007 | Prioritize (cutoff, wave, route, aging, manual) | ❌ | 1 | No priority field, no wave, no sequencing |
| WF-008 | Assign picker/team (zone, workload, role) | 🟡 | 1 | `assigned_to` + `/assign` + UI exist; no team/queue, no role/zone/workload validation |
| WF-009 | Handheld login (individual, lockout, timeout) | 🟡 | 0.5 | Worker QR login exists; lock-after-failures & session timeout not implemented |
| WF-010 | Task accept / record start time | 🟡 | 0.5 | Task exists; no explicit accept + start timestamp |
| WF-011 | Navigate to source (zone/aisle/rack/bin) | 🟡 | 1 | 3D suggest endpoint exists; guided step-by-step handheld nav deferred |
| WF-012 | Bin validation (scan source bin, wrong-bin hard stop) | ❌ | 0.5 | Scan accepts optional bin override; no wrong-bin hard stop |
| WF-013 | SKU/GTIN validation | ✅ | 0.5 | Wrong SKU hard stop, no picker bypass |
| WF-014 | Serial validation (belongs to SKU, available, not consumed) | 🟡 | 1.5 | Serial grouping + Mfg/Exp display only; no availability/consumed/blocked enforcement |
| WF-015 | Quantity confirmation (over-pick block, short-pick exception) | 🟡 | 1 | Over-pick blocked; short-pick exception flow missing |
| WF-016 | Inventory movement (status: available→picked→in-transit-to-stage) | 🟡 | 1 | Bin stock decremented + movement ledger; no status transitions; no idempotency guard |
| WF-017 | Multi-bin continuation | ✅ | 0.5 | Split-across-bins implemented |
| WF-018 | Handling unit (trolley/carton/pallet) | ❌ | 1 | No HU association during pick |
| WF-019 | Stage transfer (direct to staging lane) | ❌ | 1.5 | No staging lane concept |
| WF-020 | Stage validation (scan staging) | ❌ | 0.5 | Missing |
| WF-021 | Task completion (all lines/approved exceptions) | 🟡 | 0.5 | `complete_pick_list` exists; no approved-exception prerequisite |
| WF-022 | Status update / ERP sync (queue + retry) | 🟡 | 1 | Dispatch + invoice ref exist; no outbound message queue/retry/alert |
| WF-023 | Immutable audit (operator, device, scans, reasons, approvals) | 🟡 | 1 | `QRScanEvent` + audit log exist; no exception/approval/override audit |

> Estimates are working days for **one 10-year full-stack developer using DeepSeek V4 Pro (AI-assisted)** — FastAPI + SQLAlchemy + React/TS + handheld where applicable. `0.5` = already implemented — regression/verification only. **Total ≈ 20.5 days.**
