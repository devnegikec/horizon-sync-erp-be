...
def ensure_single_master_organization():
    ...
    """
    print(
        "🔧 ENSURING COMPLETE B2B SETUP: MASTER ORG + CUSTOMERS + CORE SYNC (THREAD-SAFE)"
    )
    print("=" * 80)
...
def cleanup_duplicate_masters(db, keep_master_id):
    ...
    for duplicate in duplicates:
        ...
        db.execute(
            text("DELETE FROM organizations WHERE id = :id"), {"id": duplicate.id}
        )
...
def consolidate_master_organizations(db, by_name, by_type):
    ...
    for duplicate_id in duplicate_ids:
        print(
            f"   → Migrating child organizations from {duplicate_id} to {primary_master_id}"
        )
        db.execute(
            text(
                """
    ...
    for org in orphan_orgs:
        ...
        if org.parent_organization_id:
            ...
            if parent_type and parent_type.organization_type != "master":
                print(
                    f"   → Preserving existing hierarchy: {org.name} under {org.parent_organization_id}"
                )
                continue
...
def sync_customer_records_to_core(master_org_id):
    ...
    try:
        ...
        customer_orgs = identity_db.execute(
            text(
                """
        ...
        for org in customer_orgs:
            ...
            customer_code = (
                org.slug if org.slug else org.name.upper().replace(" ", "_")[:20]
            )
            ...
            while True:
                ...
                """
                    ...
                    {"code": customer_code, "org_id": master_org_id},
                ).fetchone()
            ...
            if result.was_inserted:
                synced_count += 1
                print(
                    f"   ✓ Created customer: {result.customer_name} (Code: {customer_code})"
                )
            else:
                updated_count += 1
                print(
                    f"   ↻ Updated customer: {result.customer_name} (Code: {customer_code})"
                )

        core_db.commit()

        print(
            f"   ✅ Customer sync completed: {synced_count} created, {updated_count} updated"
        )
...
def verify_complete_setup(master_org_id):
    ...
    try:
        ...
        master_orgs = identity_db.execute(
            text(
                """
        ...
        if len(master_orgs) != 1:
            print(
                f"   ❌ ERROR: {len(master_orgs)} master organizations found (expected exactly 1)"
            )
            return
        ...
        try:
            customer_records = core_db.execute(
                text(
                    """
                SELECT COUNT(*) as count
                FROM customers
            ...
            if customer_orgs.count == customer_records.count:
                ...
                print(
                    f"   ⚠️  Identity ↔ Core sync: Mismatch detected ({customer_orgs.count} orgs vs {customer_records.count} customers)"
                )

            # Sample customer records
            if customer_records.count > 0:
                sample_customers = core_db.execute(
                    text(
                        """
                ...
                for customer in sample_customers:
                    location = (
                        f"{customer.city or ''}, {customer.state or ''}".strip(", ")
                        or "No location"
                    )
                    print(
                        f"      → {customer.customer_name} ({customer.customer_code}) - {location}"
                    )
            ...
            if billing_ready:
                print(
                    "   🎯 B2B Billing Status: ✅ READY - Master can bill all customers"
                )
            else:
    ...
    customer_stats = identity_db.execute(
        text(
            """
        SELECT
    ...
    customer_types = identity_db.execute(
        text(
            """
        SELECT
            organization_type,
    ...
    if customer_types:
        ...
        for ctype in customer_types:
            print(
                f"      → {ctype.organization_type}: {ctype.count} total, {ctype.direct_to_master} direct customers"
            )

    # Check for orphaned organizations
    orphaned = identity_db.execute(
        text(
            """
        SELECT COUNT(*) as count
        FROM organizations
    ...
    if orphaned.count > 0:
        ...
        orphaned_details = identity_db.execute(
            text(
                """
    ...
    hierarchy_check = identity_db.execute(
        text(
            """
    ...
    total_non_master = identity_db.execute(
        text(
            """
        SELECT COUNT(*) as count
        FROM organizations
    ...
    coverage_pct = (
        (hierarchy_check.connected_orgs / total_non_master.count * 100)
        if total_non_master.count > 0
        else 100
    )

    print(
        f"   🌳 Hierarchy Coverage: {hierarchy_check.connected_orgs}/{total_non_master.count} ({coverage_pct:.1f}%)"
    )
...
def main():
    ...
    print(
        "Includes thread-safety mechanisms: advisory locks, atomic UPSERT, constraints"
    )
    print("=" * 85)
