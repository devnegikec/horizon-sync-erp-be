...
def create_payments_with_allocations(db: Session, org_id: uuid.UUID, admin_user_id: uuid.UUID):
    ...
    for i, invoice in enumerate(customer_invoices[:5]):
        try:
            ...
            payment_data = PaymentEntryCreate(
                ...
                reference_no=f"PAY-CUST-{7000 + i}" if payment_mode != PaymentMode.CASH else None
            )
            ...
            payment_count += 1
            allocation_count += 1
            print(f"  Created payment {confirmed_payment.receipt_number} for invoice {invoice.invoice_no} - ${payment.amount}")
            
        except Exception as e:
            print(f"  Error creating payment for invoice {invoice.invoice_no}: {str(e)}")
            db.rollback()
            continue
...
def print_summary(db: Session, org_id: uuid.UUID):
    ...
    unpaid_customer_query = text("""
        ...
        AND balance_due > 0
    """)
    unpaid_customer_count = db.execute(unpaid_customer_query, {"org_id": org_id}).scalar()
    ...
    unpaid_supplier_query = text("""
        ...
        AND balance_due > 0
    """)
    unpaid_supplier_count = db.execute(unpaid_supplier_query, {"org_id": org_id}).scalar()
...
def main():
    ...
    try:
        ...
        payment_count, allocation_count = create_payments_with_allocations(db, ORG_ID, ADMIN_USER_ID)
        db.commit()
