...
class BankAccountBase(BaseModel):
    ...
    account_purpose: Optional[str] = Field(None, max_length=50, description="Account purpose (operating, payroll, etc.)")
    is_primary: bool = Field(default=False, description="Is this the primary bank account")
    is_active: bool = Field(default=True, description="Is this bank account active")
    
    # Banking features
    online_banking_enabled: bool = Field(default=False, description="Online banking enabled")
    mobile_banking_enabled: bool = Field(default=False, description="Mobile banking enabled")
    wire_transfer_enabled: bool = Field(default=False, description="Wire transfer enabled")
    ach_enabled: bool = Field(default=False, description="ACH transfers enabled")
    
    # Limits and controls
    daily_transfer_limit: Optional[float] = Field(None, gt=0, description="Daily transfer limit")
    monthly_transfer_limit: Optional[float] = Field(None, gt=0, description="Monthly transfer limit")
    requires_dual_approval: bool = Field(default=False, description="Requires dual approval for transactions")
    
    # Integration settings
    bank_api_enabled: bool = Field(default=False, description="Bank API integration enabled")
    sync_frequency: str = Field(default='manual', max_length=20, description="Sync frequency")
    ...
    def validate_iban(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v:
            ...
            if not re.match(r'^[A-Z]{2}[0-9]{2}[A-Z0-9]+$', v):
                raise ValueError('Invalid IBAN format. Expected format: CC##AAAA... (CC=country, ##=check digits, A=account identifier)')
    ...
    @classmethod
    def validate_swift(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v:
            ...
            if not re.match(r'^[A-Z]{4}[A-Z]{2}[A-Z0-9]{2}([A-Z0-9]{3})?$', v):
                raise ValueError('Invalid SWIFT code format. Expected format: AAAACCLLBBB (A=bank, C=country, L=location, B=branch)')
        return v
    ...
    @classmethod
    def validate_sort_code(cls, v: Optional[str]) -> Optional[str]:
    ...
    @classmethod
    def validate_bsb_number(cls, v: Optional[str]) -> Optional[str]:
    ...
    def validate_account_type(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v:
            allowed_types = ['checking', 'savings', 'business', 'money_market', 'certificate_deposit', 'other']
            if v.lower() not in allowed_types:
                raise ValueError(f'Invalid account type. Allowed values: {", ".join(allowed_types)}')
        return v.lower() if v else v

    @field_validator('account_purpose')
    @classmethod
    def validate_account_purpose(cls, v: Optional[str]) -> Optional[str]:
        """Validate account purpose"""
        if v:
            allowed_purposes = ['operating', 'payroll', 'tax', 'petty_cash', 'investment', 'escrow', 'other']
            if v.lower() not in allowed_purposes:
                raise ValueError(f'Invalid account purpose. Allowed values: {", ".join(allowed_purposes)}')
        return v.lower() if v else v
    ...
    def validate_sync_frequency(cls, v: str) -> str:
        ...
        if v.lower() not in allowed_frequencies:
            raise ValueError(f'Invalid sync frequency. Allowed values: {", ".join(allowed_frequencies)}')
        return v.lower()
...
class BankAccountResponse(BankAccountBase):
    ...
    def serialize_iban(self, value: Optional[str]) -> Optional[str]:
