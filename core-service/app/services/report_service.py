    ...
    def generate_chart_of_accounts_report(
        ...
        for account in accounts:
            ...
            balance_data = self.balance_calculator.calculate_balance(
                account.id, as_of_date=as_of_date, use_cache=True
            )
    ...
    def generate_hierarchical_report(
        ...
        def build_tree_node(account: Account, level: int = 0) -> dict:
            ...
            if account.id in children_map:
                for child in sorted(
                    children_map[account.id], key=lambda a: a.account_code
                ):
        ...
        tree = [
            build_tree_node(account)
            for account in sorted(root_accounts, key=lambda a: a.account_code)
        ]
