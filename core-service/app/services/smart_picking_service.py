...
class SmartPickingService:
    ...
    # ── 4. Bulk Delivery — group by warehouse ──────────────────────

    def create_bulk_delivery(
        self,
        sales_order_id: UUID,
        allocations: list[dict],
        organization_id: UUID,
        ...
        """Create one pick list per warehouse and a delivery note for each.

        Groups the incoming allocations by warehouse_id, then for each group:
        1. Creates a pick list (reserves stock)
        2. Creates a delivery note from that pick list (deducts stock, audit trail)

        Everything runs in a single DB transaction so it's all-or-nothing.
        """
        so = (
            self.db.query(SalesOrder)
            .filter(
        ...
        warehouse_groups: dict[UUID, list[dict]] = defaultdict(list)
        for alloc in allocations:
            wh_id = alloc["warehouse_id"]
            if isinstance(wh_id, str):
                wh_id = UUID(wh_id)
            warehouse_groups[wh_id].append(alloc)

        # ── Validate stock for ALL allocations upfront ──
        for alloc in allocations:
            sl = (
                self.db.query(StockLevel)
                .filter(
        ...
        # Build SO item rate map once
        so_item_rates: dict[UUID, Decimal] = {}
        for soi in so.items:
            so_item_rates[soi.item_id] = soi.rate
        ...
        for warehouse_id, wh_allocations in warehouse_groups.items():
            # ── Create pick list for this warehouse ──
            pl_idx += 1
            pick_list_no = f"PL-{now.strftime('%Y')}-{pl_base_count + pl_idx:04d}"
            ...
            pick_items: list[PickListItem] = []
            for idx, alloc in enumerate(wh_allocations):
                pli = PickListItem(
                    id=uuid.uuid4(),
                    organization_id=organization_id,
                    pick_list_id=pick_list.id,
            ...
            # ── Create delivery note for this pick list ──
            dn_no = f"DN-{now.strftime('%Y')}-{dn_base_count + pl_idx:04d}"
            ...
            for pli in pick_items:
                rate = so_item_rates.get(pli.item_id, Decimal("0"))
                amount = Decimal(str(pli.qty)) * rate
                ...
                # Deduct stock — on_hand and reserved both go down
                sl = (
                    self.db.query(StockLevel)
                    .filter(
                        StockLevel.product_id == pli.item_id,
                ...
                sm = StockMovement(
                    ...
                    reference_type="delivery_note",
                    reference_id=dn.id,
                    notes=f"Delivery from pick list {pick_list.pick_list_no}",
                    performed_by=user_id,
                    performed_at=now,
                )
                self.db.add(sm)
            ...
            # Update delivered_qty on SO items for this pick list's items
            for pli in pick_items:
                soi = (
                    self.db.query(SalesOrderItem)
                    .filter(
        ...
        return {
            "sales_order_id": so.id,
            "sales_order_no": so.sales_order_no,
            "pick_lists_created": len(warehouse_groups),
            "delivery_notes_created": len(warehouse_groups),
