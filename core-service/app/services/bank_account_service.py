...
class BankAccountService:
    ...
    def create_bank_account(
        ...
        logger.info(f"Bank account {bank_account.id} created for GL account {gl_account_id} by {current_user}")
        return bank_account
    ...
    def get_bank_account_by_id(
        ...
        if not bank_account:
            raise BankAccountNotFoundException(f"Bank account {bank_account_id} not found")
    ...
    def get_bank_accounts_by_gl_account(
        ...
        return query.order_by(BankAccount.is_primary.desc(), BankAccount.created_at).all()
    ...
    def get_banking_overview(self, organization_id: UUID) -> BankingOverviewResponse:
        ...
        total_query = (
            self.db.query(func.count(BankAccount.id))
            .filter(BankAccount.organization_id == organization_id)
        )
    ...
    def _get_gl_account_by_id(self, gl_account_id: UUID, organization_id: UUID) -> Account:
        ...
        if not gl_account:
            raise ValidationError(f"GL account {gl_account_id} not found or does not belong to organization")
    ...
    def _validate_create_rules(
        ...
        if data.iban:
            ...
            if existing_iban:
                raise DuplicateIbanException(f"IBAN {data.iban} already exists for this organization")
        ...
        if data.is_primary:
            ...
            if existing_primary:
                raise ValidationError(
                    f"GL account already has a primary bank account. "
                    f"Please deactivate the existing primary bank account first."
                )

    def _validate_update_rules(self, data: BankAccountUpdate, existing_bank_account: BankAccount) -> None:
    ...
    def _ensure_single_primary_bank_account(
        ...
        gl_account_id: UUID, 
        organization_id: UUID,
        exclude_bank_account_id: Optional[UUID] = None
    ) -> None:
