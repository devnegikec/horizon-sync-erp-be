...
class ExportService:
    ...
    def export_to_xlsx(
        ...
        for row_num, account in enumerate(report["accounts"], 2):
            ...
            ws.cell(
                row=row_num,
                column=6,
                value="Yes" if account["is_posting_account"] else "No",
            )
    ...
    def export_to_pdf(
        ...
        for account in report["accounts"]:
            table_data.append(
                [
                    account["account_code"],
                    account["account_name"][:30] + "..."
                    if len(account["account_name"]) > 30
    ...
    def export_trial_balance_to_pdf(
        ...
        # Prepare table data
        table_data = [["Code", "Name", "Type", "Debit", "Credit"]]

        for account in report["accounts"]:
            table_data.append(
                [
                    account["account_code"],
                    account["account_name"][:35] + "..."
                    if len(account["account_name"]) > 35
                    else account["account_name"],
                    account["account_type"][:10] if account["account_type"] else "",
        ...
        balance_status = (
            ...
            else f"✗ Trial Balance is OUT OF BALANCE (Difference: {report['difference']:.2f})"
        )
