...
class BalanceCalculator:
    ...
    def _get_natural_balance(
        self, account_type: AccountType, debit_total: Decimal, credit_total: Decimal
    ) -> Decimal:
        """
    ...
    def calculate_balance(
        ...
        if use_cache:
            cache_key = get_balance_cache_key(
                account_id, as_of_date.isoformat() if as_of_date else None
            )
        ...
        balance = self._get_natural_balance(
            account.account_type, debit_total, credit_total
        )
        ...
        if account.currency != base_currency:
            try:
                ...
                logger.warning(
                    f"Currency conversion failed for account {account_id}: {e}"
                )
                base_currency_balance = balance
    ...
    def calculate_consolidated_balance(
        ...
        # Get parent account
        parent_account = (
            self.db.query(Account).filter(Account.id == parent_account_id).first()
        )
    ...
    def invalidate_hierarchy_cache(self, account_id: UUID) -> int:
        ...
        logger.info(
            f"Invalidated {total_deleted} cache entries in hierarchy for account {account_id}"
        )
    ...
    def get_balance_history(
        ...
        while current_date <= end_date:
            balance_data = self.calculate_balance(
                account_id, current_date, use_cache=True
            )
            if balance_data:
