...
class InvoiceJournalPostingService:
    ...
    def reverse_invoice_journal_entry(
        self, invoice, organization_id: UUID, user_id: UUID
    ) -> dict | None:
        """
        Create reversing journal entry for cancelled invoice.
        ...
        Returns:
            Reversing journal entry as dict, or None if no original entry found

        Raises:
            ValidationError: If reversal fails
        """
        try:
            # Retrieve original journal entry for invoice
            ...
            if not original_entry:
                # No journal entry to reverse - invoice may have been cancelled before confirmation
                return None
            ...
            reversing_entry_data = {
                ...
                "status": "posted",
                "voucher_type": "Invoice Cancellation",
                "reference_type": "Invoice",
                "reference_id": invoice.id,
                "remarks": f"Reversal of invoice confirmation for {invoice.invoice_no}",
            ...
            reversing_journal_entry = self.journal_entry_service.create(
                reversing_entry_data, organization_id, user_id
            )
            ...
            raise ValidationError(
                f"Failed to reverse journal entry for invoice {invoice.invoice_no}: {str(e)}"
            )
