...
class PaymentExportService:
    ...
    def _create_summary_sheet(self, wb: Workbook, report: dict, organization_name: str):
        ...
        ws.cell(row=2, column=1, value="Payment Reconciliation Report").font = Font(
            bold=True, size=12
        )
        ...
        # Payments by Status
        row += 2
        ws.cell(row=row, column=1, value="Payments by Status").font = Font(
            bold=True, size=11
        )
        row += 1
        ...
        # Payments by Mode
        row += 2
        ws.cell(row=row, column=1, value="Payments by Mode").font = Font(
            bold=True, size=11
        )
        row += 1
    ...
    def _create_payments_detail_sheet(self, wb: Workbook, report: dict):
        ...
        for row_num, payment in enumerate(report["payments"], 2):
            ws.cell(
                row=row_num, column=1, value=payment.get("receipt_number", "")
            ).border = thin_border
            ws.cell(
                row=row_num, column=2, value=payment["payment_date"]
            ).border = thin_border
            ws.cell(
                row=row_num, column=3, value=payment["payment_type"]
            ).border = thin_border
            ws.cell(
                row=row_num, column=4, value=float(payment["amount"])
            ).border = thin_border
            ws.cell(
                row=row_num, column=5, value=payment["currency_code"]
            ).border = thin_border
            ws.cell(
                row=row_num, column=6, value=payment["payment_mode"]
            ).border = thin_border
            ws.cell(
                row=row_num, column=7, value=payment.get("reference_no", "")
            ).border = thin_border
            ...
            ws.cell(
                row=row_num, column=10, value=float(payment["unallocated_amount"])
            ).border = thin_border
            ws.cell(
                row=row_num, column=11, value=payment["allocation_count"]
            ).border = thin_border
    ...
    def _create_unallocated_sheet(self, wb: Workbook, report: dict):
        ...
        for row_num, payment in enumerate(report["unallocated_payments"], 2):
            ...
            ws.cell(
                row=row_num, column=8, value=float(payment["unallocated_amount"])
            ).border = thin_border
    ...
    def export_to_pdf(
        ...
        if filters["date_from"] and filters["date_to"]:
            metadata_parts.append(
                f"Period: {filters['date_from']} to {filters['date_to']}"
            )
        elif filters["date_from"]:
    ...
    def _add_summary_section(self, elements: list, report: dict, styles):
        ...
        summary_data = [
            ...
            [
                ...
                f"${float(summary['total_payments_received']):,.2f}",
            ],
    ...
    def _add_status_breakdown_section(self, elements: list, report: dict, styles):
        ...
        for status, data in report["payments_by_status"].items():
            status_data.append(
                [status, str(data["count"]), f"${float(data['total_amount']):,.2f}"]
            )
    ...
    def _add_mode_breakdown_section(self, elements: list, report: dict, styles):
        ...
        for mode, data in report["payments_by_mode"].items():
            mode_data.append(
                [mode, str(data["count"]), f"${float(data['total_amount']):,.2f}"]
            )
