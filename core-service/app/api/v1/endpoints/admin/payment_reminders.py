    ...
    try:
        ...
        for config in configs:
            ...
            try:
                ...
                org_name = (
                    org_detail.name
                    or org_detail.display_name
                    or f"Organization {str(config.organization_id)[:8]}"
                )
                logger.debug(
                    f"Successfully fetched organization name: {org_name} for ID: {config.organization_id}"
                )
            except Exception as e:
                logger.warning(
                    f"Failed to fetch organization name for {config.organization_id}: {e}"
                )
    ...
    **Required Permission:** system_admin.billing
    ...
    try:
        ...
        existing_config = (
            ...
            .filter(ReminderConfig.organization_id == config_data.organization_id)
            .first()
        )
        ...
        try:
            ...
            if (
                "reminder_configs_organization_id_key" in str(e)
        ...
        logger.info(
            f"Created reminder configuration {config.id} for organization {config_data.organization_id}"
        )
        return ReminderConfigResponse(**config_dict)
...
async def update_reminder_config(
    ...
    **Required Permission:** system_admin.billing
    ...
    try:
        ...
        config = reminder_service.update_reminder_config(
            config.organization_id, update_dict
        )
...
async def toggle_reminder_config(
    ...
    **Required Permission:** system_admin.billing
    ...
    try:
        ...
        logger.info(
            f"Toggled reminder configuration {config_id} to {'enabled' if config.is_enabled else 'disabled'}"
        )
        return ReminderConfigResponse(**config_dict)
...
async def list_reminder_logs(
    ...
    stage: Optional[ReminderStage] = Query(
        None, description="Filter by reminder stage"
    ),
    current_user: CurrentUser = Depends(require_permission("system_admin.billing")),
    db: Session = Depends(get_db),
):
    ...
    **Required Permission:** system_admin.billing
    ...
    try:
        ...
        query = (
            db.query(ReminderLog, Invoice.invoice_no)
        ...
        for result in results:
            ...
            log_dict[
                "organization_name"
            ] = f"Organization {str(log.organization_id)}"  # Will be improved with actual org lookup
...
async def get_reminder_stats(
    ...
    **Required Permission:** system_admin.billing
    ...
    try:
        ...
        logger.info(
            f"Retrieved reminder stats for organization {organization_id or 'all'}"
        )
        return stats
    ...
    try:
        ...
        if request.dry_run:
            ...
            result = await reminder_service.send_batch_reminders(
                organization_ids=request.organization_ids, force_send=request.force_send
            )
