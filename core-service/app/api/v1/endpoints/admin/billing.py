...
class CreateSubscriptionInvoiceRequest(BaseModel):
    ...
    credit_usage: Optional[Decimal] = Field(
        Decimal("0.00"), description="Credit usage amount"
    )
    base_price_per_seat: Optional[Decimal] = Field(
        Decimal("10.00"), description="Base price per seat"
    )
    credit_rate: Optional[Decimal] = Field(
        Decimal("0.01"), description="Price per credit unit"
    )
...
class CreateSetupFeeInvoiceRequest(BaseModel):
    ...
    description: Optional[str] = Field(
        "Initial setup fee", description="Setup fee description"
    )
    include_onboarding: Optional[bool] = Field(
        False, description="Include onboarding services"
    )
    onboarding_fee: Optional[Decimal] = Field(
        Decimal("0.00"), description="Onboarding fee amount"
    )
...
class CreateOverageInvoiceRequest(BaseModel):
    ...
    organization_id: UUID = Field(..., description="Customer organization ID")
    overage_type: str = Field(
        ..., description="Type of overage (users, credits, storage, etc)"
    )
...
class CreateCreditAdjustmentRequest(BaseModel):
    ...
    adjustment_amount: Decimal = Field(
        ..., description="Adjustment amount (positive for credits)"
    )
    reason: str = Field(..., description="Reason for adjustment")
    reference_invoice_id: Optional[UUID] = Field(
        None, description="Reference invoice if applicable"
    )
...
class SetupSystemAdminRequest(BaseModel):
    ...
    master_organization_id: UUID = Field(..., description="Master organization ID")
    admin_type: str = Field(
        "system_admin_master", description="Type of system admin role"
    )
...
@router.post(
    "/subscription-invoice", response_model=dict, status_code=status.HTTP_201_CREATED
)
async def create_subscription_invoice(
    ...
    try:
        ...
        logger.info(
            f"Created subscription invoice {invoice['id']} for org {request.organization_id}"
        )
        return invoice
...
@router.post(
    "/setup-fee-invoice", response_model=dict, status_code=status.HTTP_201_CREATED
)
async def create_setup_fee_invoice(
    ...
    try:
        ...
        invoice = service.create_setup_fee_invoice(
            ...
            onboarding_fee=request.onboarding_fee
            if request.include_onboarding
            else Decimal("0"),
        ...
        logger.info(
            f"Created setup fee invoice {invoice['id']} for org {request.organization_id}"
        )
        return invoice
...
@router.post(
    "/overage-invoice", response_model=dict, status_code=status.HTTP_201_CREATED
)
async def create_overage_invoice(
    ...
    try:
        ...
        logger.info(
            f"Created overage invoice {invoice['id']} for org {request.organization_id}"
        )
        return invoice
...
async def create_addon_invoice(
    ...
    try:
        ...
        logger.info(
            f"Created addon invoice {invoice['id']} for org {request.organization_id}"
        )
        return invoice
...
async def create_credit_adjustment_invoice(
    ...
    try:
        ...
        logger.info(
            f"Created credit adjustment invoice {invoice['id']} for org {request.organization_id}"
        )
        return invoice
...
async def get_overall_billing_summary(
    ...
    try:
        ...
        logger.info(
            f"Generated overall billing summary with {summary['total_invoices']} invoices"
        )
        return summary
...
async def get_billing_summary(
    ...
    try:
        ...
        summary = admin_service.get_organization_billing_summary(
            organization_id=customer_id, requested_by=current_user.id
        )
...
async def get_customer_organizations(
    ...
    _current_user: CurrentUser = Depends(
        require_permission("system_admin.org_manager")
    ),
):
    ...
    try:
        ...
        organizations_response = await org_service.list_organizations(
            status_filter=billing_status, page=page, page_size=limit
        )
            ...
            result.append(
                {
                    ...
                    "subscription_start_date": getattr(
                        org, "subscription_start_date", None
                    ),
                    "subscription_end_date": getattr(
                        org, "subscription_end_date", None
                    ),
                    "seat_limit": getattr(org, "max_users", None),
                    "credit_limit": getattr(org, "max_credits", None),
                    "billing_contact_email": getattr(
                        org, "billing_contact_email", None
                    ),
...
async def assign_customer_to_master(
    ...
    try:
        ...
        logger.info(
            f"Assignment request: customer={request.customer_organization_id}, master={request.master_organization_id}"
        )
...
async def setup_system_admin(
    ...
    try:
        ...
        # This should call the SystemAdminPermissionService via HTTP API
        logger.info(
            f"System admin setup request: user={request.user_id}, master_org={request.master_organization_id}"
        )
...
async def update_organization_tier(
    ...
    try:
        ...
        if not settings.identity_database_url:
            raise HTTPException(
                status_code=500, detail="Identity database not configured"
            )
