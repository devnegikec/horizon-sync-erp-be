...
def upgrade() -> None:
    ...
    op.add_column('material_requests', sa.Column('request_no', sa.String(50), nullable=True))
    op.add_column('material_requests', sa.Column(
        'type',
        sa.Enum('purchase', 'transfer', 'issue', name='materialrequesttype', create_type=False),
        nullable=False,
        server_default='purchase'
    ))
    op.add_column('material_requests', sa.Column(
        'priority',
        sa.Enum('low', 'medium', 'high', 'urgent', name='materialrequestpriority', create_type=False),
        nullable=False,
        server_default='medium'
    ))
    op.add_column('material_requests', sa.Column('target_warehouse_id', postgresql.UUID(as_uuid=True), nullable=True))
    op.add_column('material_requests', sa.Column('requested_by', postgresql.UUID(as_uuid=True), nullable=True))
    op.add_column('material_requests', sa.Column('department', sa.String(100), nullable=True))
    ...
    op.create_index('ix_material_requests_request_no', 'material_requests', ['request_no'])
    
    # Add columns to material_request_lines table
    op.add_column('material_request_lines', sa.Column('uom', sa.String(50), nullable=True))
    op.add_column('material_request_lines', sa.Column('estimated_unit_cost', sa.Numeric(15, 4), nullable=True))
    op.add_column('material_request_lines', sa.Column('requested_for', sa.String(255), nullable=True))
    op.add_column('material_request_lines', sa.Column('requested_for_department', sa.String(100), nullable=True))
