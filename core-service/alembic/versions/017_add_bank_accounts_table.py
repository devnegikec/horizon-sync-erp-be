...
def upgrade() -> None:
    ...
    if "bank_accounts" not in existing_tables:
        op.create_table(
            ...
            sa.Column(
                "bank_api_credentials_id", postgresql.UUID(as_uuid=True), nullable=True
            ),
        ...
        op.create_unique_constraint(
            "unique_iban_per_org", "bank_accounts", ["organization_id", "iban"]
        )
        op.create_index(
            "idx_bank_accounts_gl_account", "bank_accounts", ["gl_account_id"]
        )
    ...
    if "bank_account_history" not in existing_tables:
        ...
        op.create_index(
            "idx_bank_account_history_bank_account",
            "bank_account_history",
            ["bank_account_id"],
        )
        op.create_index(
            "idx_bank_account_history_action_type",
            "bank_account_history",
            ["action_type"],
        )
