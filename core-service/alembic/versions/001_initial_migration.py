...
def upgrade() -> None:
    ...
    op.execute(
        "DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'valuationmethod') THEN CREATE TYPE valuationmethod AS ENUM ('fifo', 'lifo', 'moving_average', 'standard'); END IF; END$$;"
    )
    op.execute(
        "DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'itemtype') THEN CREATE TYPE itemtype AS ENUM ('stock', 'non_stock', 'service', 'fixed_asset'); END IF; END$$;"
    )
    op.execute(
        "DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'itemstatus') THEN CREATE TYPE itemstatus AS ENUM ('active', 'inactive', 'discontinued'); END IF; END$$;"
    )
    op.execute(
        "DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'warehousetype') THEN CREATE TYPE warehousetype AS ENUM ('warehouse', 'store', 'virtual', 'transit'); END IF; END$$;"
    )

    # Create item_groups table
    if "item_groups" not in tables:
        op.create_table(
            "item_groups",
    ...
    if "warehouses_extended" not in tables:
        op.create_table(
            "warehouses_extended",
            sa.Column("id", postgresql.UUID(as_uuid=True), nullable=False),
            sa.Column("organization_id", postgresql.UUID(as_uuid=True), nullable=False),
