...
def upgrade() -> None:
    ...
    if not has_index("item_packaging_units", "idx_ipu_org"):
        op.create_index("idx_ipu_org", "item_packaging_units", ["organization_id"])
    if not has_index("item_packaging_units", "idx_ipu_item_id"):
        op.create_index("idx_ipu_item_id", "item_packaging_units", ["item_id"])
    if not has_index("item_packaging_units", "idx_ipu_qr_identifier"):
        op.create_index(
    ...
    # ══════════════════════════════════════════════════════════════════
    if not has_column("items", "sku"):
        op.add_column(
            "items",
            sa.Column("sku", sa.String(100), nullable=True),
        )
    if not has_index("items", "idx_items_sku"):
        op.create_index("idx_items_sku", "items", ["sku"])

    # ══════════════════════════════════════════════════════════════════
    # 3. Add packaging_unit_id to bin_stock_levels
    # ══════════════════════════════════════════════════════════════════
    if not has_column("bin_stock_levels", "packaging_unit_id"):
        op.add_column(
            "bin_stock_levels",
            sa.Column(
                "packaging_unit_id",
    ...
    # ══════════════════════════════════════════════════════════════════
    if not has_column("warehouse_locations", "max_volume_cc"):
        op.add_column(
            "warehouse_locations",
            sa.Column("max_volume_cc", sa.Numeric(15, 2), nullable=True),
        )
    if not has_column("warehouse_locations", "max_weight_grams"):
        op.add_column(
            "warehouse_locations",
    ...
    # ══════════════════════════════════════════════════════════════════
    if has_column("scan_session_items", "quantity") and not has_column("scan_session_items", "raw_quantity"):
        op.alter_column(
            "scan_session_items",
            "quantity",
    ...
    # ══════════════════════════════════════════════════════════════════
    if not has_column("scan_session_items", "packaging_unit_id"):
        op.add_column(
            "scan_session_items",
            sa.Column(
                "packaging_unit_id",
...
def downgrade() -> None:
    ...
    if has_column("scan_session_items", "packaging_unit_id"):
        op.drop_column("scan_session_items", "packaging_unit_id")

    # ── 5. Rename raw_quantity → quantity on scan_session_items ──
    if has_column("scan_session_items", "raw_quantity") and not has_column("scan_session_items", "quantity"):
        op.alter_column(
            "scan_session_items",
            "raw_quantity",
