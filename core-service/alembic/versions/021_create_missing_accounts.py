...
def upgrade() -> None:
    ...
    if "accounts" not in existing:
        ...
        """))
        conn.execute(sa.text("CREATE INDEX idx_accounts_organization_id ON accounts (organization_id)"))
        conn.execute(sa.text("CREATE INDEX idx_accounts_account_code ON accounts (account_code)"))
        conn.execute(sa.text("CREATE INDEX idx_accounts_account_type ON accounts (account_type)"))
        conn.execute(sa.text("CREATE INDEX idx_accounts_parent_account_id ON accounts (parent_account_id)"))
        conn.execute(sa.text("CREATE INDEX idx_accounts_status ON accounts (status)"))
        conn.execute(sa.text("CREATE INDEX idx_accounts_org_type ON accounts (organization_id, account_type)"))
        conn.execute(sa.text("CREATE INDEX idx_accounts_org_status ON accounts (organization_id, status)"))
        conn.execute(sa.text("CREATE INDEX idx_accounts_org_code ON accounts (organization_id, account_code)"))
    ...
    if "account_balances" not in existing:
        op.create_table(
            ...
            sa.Column("debit_total", sa.Numeric(19, 4), nullable=False, server_default="0"),
            sa.Column("credit_total", sa.Numeric(19, 4), nullable=False, server_default="0"),
            sa.Column("balance", sa.Numeric(19, 4), nullable=False, server_default="0"),
            sa.Column("base_currency_balance", sa.Numeric(19, 4), nullable=False, server_default="0"),
    ...
    if "payment_entries" not in existing:
        ...
        """))
        conn.execute(sa.text("CREATE INDEX idx_pe_org_id ON payment_entries (organization_id)"))
        conn.execute(sa.text("CREATE INDEX idx_pe_party ON payment_entries (party_id)"))
        conn.execute(sa.text("CREATE INDEX idx_pe_status ON payment_entries (organization_id, status)"))
        conn.execute(sa.text("CREATE INDEX idx_pe_date ON payment_entries (organization_id, payment_date)"))
        conn.execute(sa.text("CREATE INDEX idx_pe_reference ON payment_entries (reference_no)"))
        conn.execute(sa.text("CREATE INDEX idx_pe_receipt ON payment_entries (receipt_number)"))
    ...
    if "payment_references" not in existing:
        op.create_table(
            ...
            sa.Column("payment_id", UUID(as_uuid=True), sa.ForeignKey("payment_entries.id", ondelete="CASCADE"), nullable=False, index=True),
            ...
            sa.Column("exchange_rate", sa.Numeric(15, 6), nullable=True, server_default="1.0"),
            sa.Column("allocated_amount_invoice_currency", sa.Numeric(15, 2), nullable=True),
            sa.Column("created_by", UUID(as_uuid=True), nullable=False),
            sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("NOW()")),
            sa.UniqueConstraint("payment_id", "invoice_id", name="unique_payment_references_payment_invoice"),
        )
    ...
    if "bank_account_history" not in existing:
        op.create_table(
            "bank_account_history",
            sa.Column("id", UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
            sa.Column("bank_account_id", UUID(as_uuid=True), nullable=False, index=True),
...
def downgrade() -> None:
    ...
    for tbl in [
        "bank_account_history", "bank_accounts",
        "payment_audit_log", "payment_references", "payment_entries",
        "account_audit_log", "default_accounts", "account_balances",
        "accounts",
    ]:
        if tbl in existing:
    ...
    for enum in [
        "payment_audit_action", "payment_source",
        "payment_status", "payment_mode", "payment_type",
    ]:
