...
def upgrade() -> None:
    ...
    if not has_column("items", "qr_product_id"):
        op.add_column(
            "items",
            sa.Column(
                "qr_product_id",
    ...
    if not has_index("items", "ix_items_qr_product_id"):
        op.create_index(
    ...
    if not has_constraint("items", "fk_items_qr_product_id"):
...
def downgrade() -> None:
    if has_constraint("items", "fk_items_qr_product_id"):
        op.drop_constraint("fk_items_qr_product_id", "items", type_="foreignkey")
    if has_index("items", "ix_items_qr_product_id"):
        op.drop_index("ix_items_qr_product_id", table_name="items")
    if has_column("items", "qr_product_id"):
