...
def upgrade() -> None:
    ...
    # Tables/indexes may already exist where SQLAlchemy's create_all() ran at
    # app startup before migrations; guard each DDL so this stays idempotent.
    if not _table_exists(conn, "packing_slips"):
        op.create_table(
            "packing_slips",
    ...
    if not _index_exists(conn, "ix_packing_slips_organization_id"):
        op.create_index(
            "ix_packing_slips_organization_id", "packing_slips", ["organization_id"]
        )

    if not _table_exists(conn, "packing_slip_items"):
        op.create_table(
            "packing_slip_items",
            sa.Column("id", postgresql.UUID(as_uuid=True), nullable=False),
            sa.Column("organization_id", postgresql.UUID(as_uuid=True), nullable=False),
    ...
    if not _index_exists(conn, "ix_packing_slip_items_organization_id"):
        op.create_index(
            "ix_packing_slip_items_organization_id",
            "packing_slip_items",
            ["organization_id"],
        )
    if not _index_exists(conn, "ix_packing_slip_items_order_id"):
        op.create_index(
            "ix_packing_slip_items_order_id", "packing_slip_items", ["order_id"]
        )
    if not _index_exists(conn, "ix_packing_slip_items_pick_list_id"):
        op.create_index(
            "ix_packing_slip_items_pick_list_id",
            "packing_slip_items",
            ["pick_list_id"],
        )
