...
def test_opening_balance_journal_entry_logic():
    ...
    is_debit_balance = asset_account.account_type in (MockAccountType.ASSET, MockAccountType.EXPENSE)
    ...
    assert je_lines[0]['debit'] == opening_balance, "Asset account should be debited with opening balance"
    assert je_lines[1]['credit'] == opening_balance, "Equity account should be credited to balance"
    ...
    opening_amount = Decimal(str(opening_balance))
    is_debit_balance = liability_account.account_type in (MockAccountType.ASSET, MockAccountType.EXPENSE)
    ...
    print(f"  Account: {liability_account.account_code} ({liability_account.account_name})")
    print(f"  Opening Balance: ${opening_balance}")
    print(f"  Journal Entry Lines:")
    for i, line in enumerate(je_lines, 1):
    ...
    assert total_debits == total_credits, "Journal entry must be balanced!"
    assert je_lines[0]['credit'] == opening_balance, "Liability account should be credited with opening balance"
    assert je_lines[1]['debit'] == opening_balance, "Equity account should be debited to balance"
...
def test_opening_balance_equity_account_logic():
    ...
    print(f"  Opening Balance Equity Account Structure:")
    ...
    assert obe_account["account_type"] == MockAccountType.EQUITY, "OBE should be an equity account"
...
def test_balance_calculation_logic():
    ...
    journal_entries = [
        ...
        {"account_id": account_id, "debit": 1000.0, "credit": 0.0, "type": "Opening Balance"},
        # Some transactions
        {"account_id": account_id, "debit": 500.0, "credit": 0.0, "type": "Cash Receipt"},
        {"account_id": account_id, "debit": 0.0, "credit": 200.0, "type": "Cash Payment"},
    ]
    ...
    print(f"  Asset Account Journal Entries:")
    for entry in journal_entries:
        print(f"    {entry['type']}: Debit ${entry['debit']:.2f}, Credit ${entry['credit']:.2f}")
    
    print(f"  Balance Calculation (Asset Account):")
    print(f"    Total Debits:  ${total_debits:.2f}")
    print(f"    Total Credits: ${total_credits:.2f}")
    print(f"    Balance (Debit - Credit): ${current_balance:.2f}")
    
    expected_balance = 1300.0  # 1000 + 500 - 200
    assert current_balance == expected_balance, f"Expected balance {expected_balance}, got {current_balance}"
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("  ✅ Balance calculation will show correct values based on journal entries")
