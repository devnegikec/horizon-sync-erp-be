...
def test_balance_population_logic():
    ...
    assert isinstance(account_response["current_balance"], (int, float)), (
    ...
    assert "balance" in balance_calculator_result, (
        "❌ Balance calculator missing balance"
    )
    assert "debit_total" in balance_calculator_result, (
        "❌ Balance calculator missing debit_total"
    )
    assert "credit_total" in balance_calculator_result, (
        "❌ Balance calculator missing credit_total"
    )
...
def test_level_hierarchy_logic():
    ...
    parent = MockAccount(
        account_code="1000", account_name="Current Assets", level=1, is_group=True
    )
    ...
    assert grandchild.level == 3, (
        f"❌ Grandchild level should be 3, got {grandchild.level}"
    )
    ...
    assert isinstance(account_list_item["level"], int), (
        "❌ Level field should be integer"
    )
...
def test_group_hierarchy_logic():
    ...
    # Test group flags
    assert parent_account.is_group is True, (
        "❌ Parent account should be marked as group"
    )
    assert child_account.is_group is False, (
    ...
    assert "is_group" in parent_response, (
        "❌ is_group field missing from parent response"
    )
...
def test_pagination_logic():
    ...
    assert isinstance(pagination_metadata["page_size"], int), (
        "❌ Page size should be integer"
    )
    assert isinstance(pagination_metadata["total_count"], int), (
        "❌ Total count should be integer"
    )
    assert isinstance(pagination_metadata["total_pages"], int), (
        "❌ Total pages should be integer"
    )
    assert isinstance(pagination_metadata["has_next"], bool), (
        "❌ Has next should be boolean"
    )
    assert isinstance(pagination_metadata["has_previous"], bool), (
        "❌ Has previous should be boolean"
    )
    ...
    assert expected_total_pages == 3, (
        f"❌ Expected 3 pages for 25 items with page size 10, got {expected_total_pages}"
    )
...
def test_parent_name_logic():
    ...
    parent = MockAccount(account_code="7000", account_name="Operating Expenses Parent")

    child = MockAccount(
        account_code="7100", account_name="Office Rent", parent_account_id=parent.id
    )
    ...
    assert "parent_account_id" in account_list_item, (
        "❌ Account list missing parent_account_id"
    )
    assert account_list_item["parent_account_id"] == str(parent.id), (
        "❌ Incorrect parent account ID"
    )
