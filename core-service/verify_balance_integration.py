    ...
    try:
        ...
        if expected_non_zero == 'debit':
            if balance['debit_total'] > 0:
                print(f"  ✓ PASS: Debit total is non-zero (${balance['debit_total']:,.2f})")
                return True
            else:
                print(f"  ✗ FAIL: Expected non-zero debit total, got ${balance['debit_total']:,.2f}")
                return False
        elif expected_non_zero == 'credit':
            if balance['credit_total'] > 0:
                print(f"  ✓ PASS: Credit total is non-zero (${balance['credit_total']:,.2f})")
                return True
            else:
                print(f"  ✗ FAIL: Expected non-zero credit total, got ${balance['credit_total']:,.2f}")
                return False
        else:
            if balance['debit_total'] > 0 or balance['credit_total'] > 0:
                print(f"  ✓ PASS: Balance is non-zero")
                return True
            else:
                print(f"  ✗ FAIL: Expected non-zero balance, got zero")
                return False
