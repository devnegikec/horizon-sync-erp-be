...
from app.models.product_item import ProductItem  # noqa: F401 - registers relationship mapper
from app.models.qr_block import QRBlock
from app.models.qr_product import QRProduct
from app.models.qr_scan_event import QRScanEvent
from app.models.product_sku import ProductSKU  # noqa: F401 - registers relationship mapper
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Seed QSeal analytics demo scan events")
    parser.add_argument("--organization-id", required=True, help="Tenant organization UUID to seed")
    parser.add_argument("--days", type=int, default=90, choices=[30, 60, 90], help="Number of days of history to create")
    parser.add_argument("--events-per-day", type=int, default=3, choices=[1, 2, 3, 4, 5], help="Events per product per day")
    return parser.parse_args()


def make_event_id(organization_id: uuid.UUID, product_id: uuid.UUID, day: int, index: int) -> uuid.UUID:
    return uuid.uuid5(uuid.NAMESPACE_URL, f"{SEED_NAME}:{organization_id}:{product_id}:{day}:{index}")
...
def seed(organization_id: uuid.UUID, days: int, events_per_day: int) -> tuple[int, int]:
    ...
    try:
        products = (
            db.query(QRProduct)
            .filter(QRProduct.organization_id == organization_id, QRProduct.deleted_at.is_(None))
            .order_by(QRProduct.created_at)
            .limit(10)
            .all()
        )
        if not products:
            raise RuntimeError(f"No QR products found for organization {organization_id}. Create a QSeal product first.")

        product_ids = [product.id for product in products]
        existing_ids = {
            row[0]
            for row in db.query(QRScanEvent.event_id)
            .filter(QRScanEvent.organization_id == organization_id, QRScanEvent.event_id.is_not(None))
            .all()
        }
        ...
        for product_index, product in enumerate(products):
            block = (
                db.query(QRBlock)
                .filter(QRBlock.organization_id == organization_id, QRBlock.product_id == product.id, QRBlock.deleted_at.is_(None))
                .order_by(QRBlock.created_at)
                .first()
            )
            batch = block.batch if block and block.batch else f"DEMO-{product_index + 1:02d}"
            serials = [f"DEMO-{product_index + 1:02d}-{number:04d}" for number in range(1, 7)]

            for day in range(days):
                for event_index in range(events_per_day):
                    event_id = make_event_id(organization_id, product.id, day, event_index)
                    if event_id in existing_ids:
                        continue

                    location = LOCATIONS[(day + event_index + product_index) % len(LOCATIONS)]
                    device = DEVICES[(day + event_index) % len(DEVICES)]
                    is_invalid = (day + event_index + product_index) % 11 == 0
                    scan_time = now - timedelta(days=day, hours=(event_index * 3 + product_index) % 12)
                    ...
                    events.append(
                        QRScanEvent(
                            ...
                            batch=batch,
                            qseal_type=QSEAL_TYPES[(day + product_index) % len(QSEAL_TYPES)],
                            ...
                            referrer_url="https://demo.brandwise.local/product" if event_index == 0 else None,
                            language="en-US",
                            is_bot=False,
...
def main() -> int:
    ...
    print("The operation is idempotent; running it again will not duplicate these events.")
    return 0
