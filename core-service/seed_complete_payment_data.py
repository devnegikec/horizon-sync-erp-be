...
def get_unpaid_invoices(
    db: Session, org_id: uuid.UUID, invoice_type: str, limit: int = 20
):
    """Get unpaid invoices"""
    query = text("""
    ...
    """)
    return db.execute(
        query, {"org_id": org_id, "invoice_type": invoice_type, "limit": limit}
    ).fetchall()


def create_draft_payments_unallocated(
...
def create_draft_payments_with_allocations(
    ...
    for i, invoice in enumerate(customer_invoices[5:10]):
        try:
            ...
            allocated_count += 1
            print(
                f"  Created payment {payment.id} with allocation to {invoice.invoice_no} - ${payment.amount}"
            )
...
def create_supplier_payments(db: Session, org_id: uuid.UUID, admin_user_id: uuid.UUID):
    ...
    for i, invoice in enumerate(supplier_invoices[3:5]):
        try:
            ...
            supplier_count += 1
            print(
                f"  Created supplier payment {payment.id} with allocation - ${payment.amount}"
            )
...
def main():
    ...
    try:
        ...
        allocated_count = create_draft_payments_with_allocations(
