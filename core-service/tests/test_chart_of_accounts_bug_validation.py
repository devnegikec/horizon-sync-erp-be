...
class TestChartOfAccountBalancePopulation:
    ...
    def test_list_accounts_includes_balance_in_response(
        self, client, db_session, sample_organization
    ):
        """
        ...
        cash_account_response = next(
            (a for a in accounts if a["account_code"] == "1001"), None
        )
        revenue_account_response = next(
            (a for a in accounts if a["account_code"] == "4001"), None
        )

        assert cash_account_response is not None, "Cash account should be in response"
        assert revenue_account_response is not None, (
            "Revenue account should be in response"
        )
        ...
        assert "current_balance" in cash_account_response or hasattr(
            cash_account_response, "current_balance"
        )
        assert "opening_balance" in cash_account_response or hasattr(
            cash_account_response, "opening_balance"
        )

    def test_individual_account_get_includes_balance(
        self, client, db_session, sample_organization
    ):
        """
...
class TestChartOfAccountHierarchyLevels:
    ...
    def test_account_levels_calculated_correctly(
        self, client, db_session, sample_organization
    ):
        """
        ...
        parent_response = next(
            (a for a in accounts if a["account_code"] == "1000"), None
        )
        child_response = next(
            (a for a in accounts if a["account_code"] == "1100"), None
        )
        grandchild_response = next(
            (a for a in accounts if a["account_code"] == "1101"), None
        )
        ...
        assert parent_response["level"] == 1, (
            f"Parent should be level 1, got {parent_response.get('level')}"
        )
        assert child_response["level"] == 2, (
            f"Child should be level 2, got {child_response.get('level')}"
        )
        assert grandchild_response["level"] == 3, (
            f"Grandchild should be level 3, got {grandchild_response.get('level')}"
        )
    ...
    def test_level_calculation_during_account_creation(self, client):
        ...
        assert child_data_response["level"] == 2, (
            f"Child should be level 2, got {child_data_response.get('level')}"
        )
        assert child_data_response["parent_account_id"] == parent_id

    def test_level_appears_in_account_list_schema(
        self, client, db_session, sample_organization
    ):
        """
        ...
        assert "level" in test_account, (
            "Level field should be present in account list response"
        )
...
class TestChartOfAccountGroupHierarchy:
    ...
    def test_is_group_flag_populated_correctly(
        self, client, db_session, sample_organization
    ):
        """
        ...
        parent_response = next(
            (a for a in accounts if a["account_code"] == "4000"), None
        )
        child_response = next(
            (a for a in accounts if a["account_code"] == "4100"), None
        )
        ...
        assert parent_response["is_group"] is True, (
            "Parent account should be marked as group"
        )
        assert child_response["is_group"] is False, (
            "Child posting account should not be marked as group"
        )
    ...
    def test_group_hierarchy_with_tree_structure(self, client):
        ...
        posting_account = next(
            (a for a in accounts if a["account_code"] == "5110"), None
        )
...
class TestChartOfAccountPagination:
    ...
    def test_pagination_page_navigation(self, client, db_session, sample_organization):
        ...
        assert (
            len(page3_data["chart_of_accounts"]) >= 5
        )  # Last page may have remaining accounts
    ...
    def test_pagination_with_filters(self, client, db_session, sample_organization):
        ...
        response = client.get(
            "/api/v1/chart-of-accounts?account_type=asset&page=1&page_size=5"
        )
        assert response.status_code == status.HTTP_200_OK
        ...
        page2_response = client.get(
            "/api/v1/chart-of-accounts?account_type=asset&page=2&page_size=5"
        )
        assert page2_response.status_code == status.HTTP_200_OK
        page2_data = page2_response.json()
...
class TestEditAccountDialogParentName:
    ...
    def test_account_response_includes_parent_info(
        self, client, db_session, sample_organization
    ):
        """
    ...
    def test_account_list_includes_parent_names(
        self, client, db_session, sample_organization
    ):
        """
        ...
        # Find child accounts
        child1_response = next(
            (a for a in accounts if a["account_code"] == "8100"), None
        )
        child2_response = next(
            (a for a in accounts if a["account_code"] == "8200"), None
        )
        parent_response = next(
            (a for a in accounts if a["account_code"] == "8000"), None
        )
    ...
    def test_account_hierarchy_endpoint(self, client, db_session, sample_organization):
        ...
        hierarchy_response = client.get(
            f"/api/v1/chart-of-accounts/{leaf.id}/hierarchy"
        )
        ...
        if hierarchy_response.status_code == status.HTTP_200_OK:
            ...
            middle_hierarchy = client.get(
                f"/api/v1/chart-of-accounts/{middle.id}/hierarchy"
            )
            if middle_hierarchy.status_code == status.HTTP_200_OK:
                ...
                root_ancestor = next(
                    (
                        a
                        for a in middle_data["ancestors"]
                        if a["account_code"] == "9000"
                    ),
                    None,
                )
                ...
                leaf_child = next(
                    (c for c in middle_data["children"] if c["account_code"] == "9110"),
                    None,
                )
                assert leaf_child is not None

    def test_update_account_preserves_parent_info(
        self, client, db_session, sample_organization
    ):
        """
        ...
        update_data = {"account_name": "Updated Child Account"}
