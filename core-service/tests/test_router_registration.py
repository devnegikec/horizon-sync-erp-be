...
def test_quotation_routes_registered():
    ...
    assert len(quotation_routes) >= 4, (
        f"Expected at least 4 quotation routes, found {len(quotation_routes)}"
    )

    # Verify the routes exist
    route_paths = [str(r.path) for r in quotation_routes]
    assert any("/quotations" == path for path in route_paths), (
        "Missing /quotations route"
    )
    assert any("/quotations/{quotation_id}" in path for path in route_paths), (
        "Missing /quotations/{quotation_id} route"
    )
...
def test_sales_order_routes_registered():
    ...
    assert len(sales_order_routes) >= 5, (
        f"Expected at least 5 sales order routes, found {len(sales_order_routes)}"
    )

    # Verify the routes exist
    route_paths = [str(r.path) for r in sales_order_routes]
    assert any("/sales-orders" == path for path in route_paths), (
        "Missing /sales-orders route"
    )
    assert any("/sales-orders/{sales_order_id}" in path for path in route_paths), (
        "Missing /sales-orders/{sales_order_id} route"
    )
...
def test_router_url_prefixes():
    ...
    for route in quotation_routes:
        assert str(route.path).startswith("/quotations"), (
            f"Quotation route has incorrect prefix: {route.path}"
        )

    # Check sales-orders prefix
    sales_order_routes = [r for r in all_routes if "/sales-orders" in str(r.path)]
    for route in sales_order_routes:
        assert str(route.path).startswith("/sales-orders"), (
            f"Sales order route has incorrect prefix: {route.path}"
        )
    ...
    for expected_path, expected_methods in expected:
        ...
        for method in expected_methods:
            assert method in all_methods, (
                f"Missing method {method} for route {expected_path}"
            )
...
def test_rfq_routes_registered():
    ...
    assert len(rfq_routes) >= 8, (
        f"Expected at least 8 RFQ routes, found {len(rfq_routes)}"
    )

    # Verify the routes exist
    route_paths = [str(r.path) for r in rfq_routes]
    assert any("/rfqs" == path for path in route_paths), "Missing /rfqs route"
    assert any("/rfqs/{rfq_id}" in path for path in route_paths), (
        "Missing /rfqs/{rfq_id} route"
    )
...
def test_material_request_routes_registered():
    ...
    assert len(mr_routes) >= 6, (
        f"Expected at least 6 Material Request routes, found {len(mr_routes)}"
    )

    # Verify the routes exist
    route_paths = [str(r.path) for r in mr_routes]
    assert any("/material-requests" == path for path in route_paths), (
        "Missing /material-requests route"
    )
    assert any(
        "/material-requests/{material_request_id}" in path for path in route_paths
    ), "Missing /material-requests/{material_request_id} route"
