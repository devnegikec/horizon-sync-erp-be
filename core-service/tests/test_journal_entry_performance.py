...
from app.models.base import AccountType, AccountStatus, JournalStatus
...
def large_journal_entry_dataset(db_session, test_account, mock_current_user):
    ...
    count = db_session.query(JournalEntryLine).filter(
        JournalEntryLine.account_id == test_account.id
    ).count()
...
def test_balance_query_performance_with_many_lines(
    ...
    result = balance_calculator.calculate_balance(
