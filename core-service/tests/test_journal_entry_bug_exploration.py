...
class TestJournalEntryBugExploration:
    ...
    def test_balance_calculator_returns_zero_for_missing_tables(self, db_session, mock_current_user):
        """
    ...
    def test_journal_entry_models_not_in_app_models(self):
        ...
        **Validates: Requirements 2.3**
        ...
        assert hasattr(app.models, 'JournalEntry'), "JournalEntry should be exported from app.models"
        assert hasattr(app.models, 'JournalEntryLine'), "JournalEntryLine should be exported from app.models"
    ...
    def test_alembic_cannot_detect_journal_entry_models(self, db_session):
        ...
        **Validates: Requirements 2.3**
        ...
        assert "journal_entry_lines" in Base.metadata.tables, "Models are in Base.metadata"
        
        # After fix: they ARE in app.models exports (which is what Alembic checks)
        import app.models
        assert hasattr(app.models, 'JournalEntry'), "JournalEntry exported from app.models"
        assert hasattr(app.models, 'JournalEntryLine'), "JournalEntryLine exported from app.models"
    ...
    @given(
        ...
        account_name=st.text(min_size=3, max_size=50, alphabet=st.characters(whitelist_categories=("Lu", "Ll", "Zs"))),
    ...
    def test_property_all_accounts_return_zero_balance(self, db_session, mock_current_user, account_code, account_name, account_type):
        """
        ...
        **Validates: Requirements 1.1, 1.2, 1.3**
        ...
        assert balance_data is not None, f"Balance data should not be None for account {account.account_code}"
        assert balance_data["debit_total"] == 0.0, f"Expected debit_total=0 for {account.account_code}, got {balance_data['debit_total']}"
        assert balance_data["credit_total"] == 0.0, f"Expected credit_total=0 for {account.account_code}, got {balance_data['credit_total']}"
        assert balance_data["balance"] == 0.0, f"Expected balance=0 for {account.account_code}, got {balance_data['balance']}"
        
        # Clean up
        db_session.delete(account)
        db_session.commit()
    
    def test_counterexample_cash_account_should_have_balance(self, db_session, mock_current_user):
        """
        ...
        **Validates: Requirements 1.1, 1.2, 1.3**
        ...
        assert current_balance == 0.0, "On unfixed code, balance should be zero due to missing tables"
