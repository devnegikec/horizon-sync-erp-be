...
from app.core.cache import cache, get_balance_cache_key
...
class TestBalanceCalculationPreservation:
    ...
    def test_property_natural_balance_direction_preserved(
        self, db_session, debit_amount, credit_amount, account_type
    ):
        """
    ...
    def test_natural_balance_asset_account(self, db_session):
        ...
        **Validates: Requirements 3.2**
        ...
        balance = calculator._get_natural_balance(
            AccountType.ASSET, Decimal("1000.00"), Decimal("300.00")
        )
    ...
    def test_natural_balance_liability_account(self, db_session):
        ...
        **Validates: Requirements 3.2**
        ...
        balance = calculator._get_natural_balance(
            AccountType.LIABILITY, Decimal("300.00"), Decimal("1000.00")
        )

        assert balance == Decimal("700.00"), (
            "Liability balance should be Credit - Debit"
        )
    ...
    def test_natural_balance_expense_account(self, db_session):
        ...
        **Validates: Requirements 3.2**
        ...
        balance = calculator._get_natural_balance(
            AccountType.EXPENSE, Decimal("500.00"), Decimal("100.00")
        )
    ...
    def test_natural_balance_revenue_account(self, db_session):
        ...
        **Validates: Requirements 3.2**
        ...
        balance = calculator._get_natural_balance(
            AccountType.REVENUE, Decimal("100.00"), Decimal("500.00")
        )
    ...
    def test_natural_balance_equity_account(self, db_session):
        ...
        **Validates: Requirements 3.2**
        ...
        balance = calculator._get_natural_balance(
            AccountType.EQUITY, Decimal("200.00"), Decimal("800.00")
        )
...
class TestCurrencyConversionPreservation:
    ...
    def test_currency_service_integration_preserved(
        ...
        **Validates: Requirements 3.3**
        ...
        with patch.object(
            CurrencyService, "convert", return_value=Decimal("100.00")
        ) as mock_convert:
    ...
    def test_currency_conversion_fallback_preserved(
        self, db_session, mock_current_user
    ):
        """
        ...
        **Validates: Requirements 3.3**
        ...
        with patch.object(
            CurrencyService, "convert", side_effect=Exception("Conversion failed")
        ):
            with patch.object(CurrencyService, "get_base_currency", return_value="USD"):
                calculator = BalanceCalculator(db_session)
                balance_data = calculator.calculate_balance(account.id, use_cache=False)
...
class TestHierarchyManagerPreservation:
    ...
    def test_consolidated_balance_uses_hierarchy_manager(
        self, db_session, mock_current_user
    ):
        """
        ...
        **Validates: Requirements 3.4**
        ...
        with patch.object(
            HierarchyManager, "get_descendants", return_value=[]
        ) as mock_get_descendants:
            calculator = BalanceCalculator(db_session)
            consolidated_data = calculator.calculate_consolidated_balance(
                parent_account.id, mock_current_user.organization_id, use_cache=False
            )

        # Verify hierarchy manager was called
        mock_get_descendants.assert_called_once_with(
            parent_account.id, mock_current_user.organization_id
        )

    def test_consolidated_balance_aggregates_children(
        self, db_session, mock_current_user
    ):
        """
    ...
    def test_consolidated_balance_no_descendants_fallback(
        self, db_session, mock_current_user
    ):
        """
...
class TestCachingPreservation:
    ...
    def test_cache_key_generation_preserved(self, db_session, mock_current_user):
        ...
        **Validates: Requirements 3.7**
        ...
        cache_key_specific = get_balance_cache_key(
            account.id, specific_date.isoformat()
        )
...
class TestZeroBalancePreservation:
    ...
    def test_property_zero_balance_for_empty_accounts(
        self, db_session, mock_current_user, account_code, account_type
    ):
        """
        ...
        **Validates: Requirements 3.1**
        ...
        assert balance_data["debit_total"] == 0.0, (
            f"Expected debit_total=0 for empty account {account.account_code}"
        )
        assert balance_data["credit_total"] == 0.0, (
            f"Expected credit_total=0 for empty account {account.account_code}"
        )
        assert balance_data["balance"] == 0.0, (
            f"Expected balance=0 for empty account {account.account_code}"
        )
