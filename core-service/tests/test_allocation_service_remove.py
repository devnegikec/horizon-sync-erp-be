...
def test_remove_allocation_success(
    ...
    deallocate_log = next(
        (log for log in audit_logs if log.action == PaymentAuditAction.DEALLOCATE), None
    )
...
def test_remove_allocation_payment_not_draft(
    db_session: Session, test_organization_id: uuid.UUID, test_user_id: uuid.UUID
):
...
def test_remove_allocation_not_found(
    db_session: Session, test_organization_id: uuid.UUID, test_user_id: uuid.UUID
):
...
def test_remove_allocation_different_organization(
...
def test_remove_allocation_updates_unallocated_amount(
    db_session: Session, test_organization_id: uuid.UUID, test_user_id: uuid.UUID
):
