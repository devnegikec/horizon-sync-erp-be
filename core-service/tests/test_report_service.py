...
def test_generate_financial_statement_grouped_with_accounts(db_session, sample_accounts):
    ...
    with patch.object(service.balance_calculator, 'calculate_balance', return_value=mock_balance):
        report = service.generate_financial_statement_grouped(
            organization_id=organization_id,
            as_of_date=date.today()
        )
    ...
    for group in report["groups"]:
        assert group["account_type"] in ["asset", "liability", "equity", "income", "expense"]
