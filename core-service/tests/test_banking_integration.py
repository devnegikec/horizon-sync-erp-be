...
class TestBankingValidation:
    ...
    def test_iban_validation(self):
        ...
        for iban in valid_ibans:
            assert BankingValidator.validate_iban_checksum(iban), f"Valid IBAN should pass: {iban}"
        ...
        for iban in invalid_ibans:
            assert not BankingValidator.validate_iban_checksum(iban), f"Invalid IBAN should fail: {iban}"
    ...
    def test_swift_validation(self):
        ...
        for swift in valid_swifts:
            assert BankingValidator.validate_swift_code_format(swift), f"Valid SWIFT should pass: {swift}"
    ...
    def test_routing_number_validation(self):
        ...
        for routing in valid_routing:
            assert BankingValidator.validate_routing_number_checksum(routing), f"Valid routing should pass: {routing}"
...
class TestBankingSecurityUtils:
    ...
    def test_audit_hash_generation(self):
        ...
        assert not BankingSecurityUtils.verify_audit_hash(modified_data, secret_key, hash1)
    ...
    def test_organization_access_validation(self):
        ...
        assert not BankingSecurityUtils.validate_organization_access(user_org, different_org)
    ...
    def test_banking_permissions(self):
        ...
        assert BankingSecurityUtils.check_banking_permissions(accountant_roles, "view:banking_summary")
        assert not BankingSecurityUtils.check_banking_permissions(accountant_roles, "view:banking_details")
        
        # Finance manager can view details but not delete
        assert BankingSecurityUtils.check_banking_permissions(finance_manager_roles, "view:banking_details")
        assert not BankingSecurityUtils.check_banking_permissions(finance_manager_roles, "delete:banking_links")
        
        # Treasurer has most permissions
        assert BankingSecurityUtils.check_banking_permissions(treasurer_roles, "delete:banking_links")
        assert BankingSecurityUtils.check_banking_permissions(treasurer_roles, "manage:banking_api")
        
        # Admin has all permissions
        assert BankingSecurityUtils.check_banking_permissions(admin_roles, "manage:banking_api")
...
def test_banking_integration_workflow():
    ...
    BankAccountData = namedtuple('BankAccountData', [
        'bank_name', 'account_holder_name', 'account_number', 'iban', 
        'swift_code', 'is_primary', 'account_type'
    ])
...
def test_banking_validation_integration():
    ...
    for case in test_cases:
        ...
        if case["should_pass"]:
            ...
            assert swift_valid, f"SWIFT should be valid for {case['description']}: {swift}"
        else:
            # For this test, we expect IBAN to be invalid
            assert not iban_valid, f"IBAN should be invalid for {case['description']}: {iban}"
