...
def test_update_payment_entry_success(db_session):
    ...
    result = service.update_payment_entry(payment_entry.id, update_data, org_id, user_id)
...
def test_update_payment_entry_invalid_future_date(db_session):
    ...
    with pytest.raises(PydanticValidationError, match="cannot be more than 30 days in the future"):
