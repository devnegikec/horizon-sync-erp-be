...
class TestJournalEntryServiceIntegration:
    ...
    def test_create_and_post_journal_entry(
        ...
        cash_account = next(
            acc for acc in sample_accounts if acc.account_code == "1000"
        )
        equity_account = next(
            acc for acc in sample_accounts if acc.account_code == "3000"
        )
        ...
        assert cash_balance["credit_total"] == Decimal("0.00")
        assert cash_balance["balance"] == Decimal(
            "1000.00"
        ...
        assert equity_balance["balance"] == Decimal(
            "1000.00"
        )  # Credit - Debit for equity
    ...
    def test_multiple_journal_entries_balance_accumulation(
        ...
        revenue_account = next(
            acc for acc in sample_accounts if acc.account_code == "4000"
        )
        expense_account = next(
            acc for acc in sample_accounts if acc.account_code == "5000"
        )
