...
def test_create_account_with_opening_balance(client, auth_headers):
    ...
    response = client.post(
        "/api/v1/chart-of-accounts", json=account_data, headers=auth_headers
    )
    ...
    response = client.get(
        f"/api/v1/chart-of-accounts/{account_id}", headers=auth_headers
    )
...
def test_liability_opening_balance_creates_credit_entry(
...
def test_balance_calculator_includes_opening_balance(client, auth_headers):
    ...
    response = client.get("/api/v1/chart-of-accounts", headers=auth_headers)
