...
def create_test_account_hierarchy(
    db_session: Session, organization_id, base_code: str = "TEST"
):
    """
...
def create_test_accounts_for_pagination(
    db_session: Session, organization_id, count: int = 20
):
    """
...
def assert_parent_info_complete(account_data: dict):
    ...
    if account_data.get("parent_account_id"):
        ...
        assert "parent" in account_data, (
            "Account with parent should include parent info"
        )
