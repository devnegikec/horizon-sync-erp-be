...
def test_multi_tenancy_bug_same_account_id(db_session):
    ...
    assert balance["debit_total"] == 1000.00, (
        "Balance should only include entries from the account's organization"
    )
    assert balance["balance"] == 1000.00, (
        "Balance calculation should filter by organization_id"
    )
