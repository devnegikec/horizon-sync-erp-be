...
def update_account_defaults():
    ...
    try:
        ...
        if 'level' not in existing_cols:
            cursor.execute("ALTER TABLE accounts ADD COLUMN level INTEGER NOT NULL DEFAULT 1")
            print("Added level column")
            
        if 'is_group' not in existing_cols:
            cursor.execute("ALTER TABLE accounts ADD COLUMN is_group BOOLEAN NOT NULL DEFAULT false")
        ...
        print(f"Updated {group_updated} accounts to is_group = true (accounts with children)")
