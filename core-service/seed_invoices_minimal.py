...
def main():
    ...
    try:
        ...
        customer_check = text("SELECT id FROM customers WHERE organization_id = :org_id LIMIT 10")
        ...
        supplier_check = text("SELECT id FROM suppliers WHERE organization_id = :org_id LIMIT 5")
        ...
        for i, customer in enumerate(customers[:10]):
            for j in range(1 if i < 5 else 2):  # 5 customers get 1 invoice, 5 get 2 invoices
