...
DB_URL = os.getenv("DATABASE_URL", "postgresql://horizon_user:horizon_pass@localhost:5432/core_db")
...
for gname, gcode in groups_needed.items():
    exists = db.execute(
        text("SELECT id FROM item_groups WHERE organization_id = :org AND name = :name"),
        {"org": ORG_ID, "name": gname},
    ).fetchone()
    if not exists:
        gid = str(uuid4())
        db.execute(
            text("""INSERT INTO item_groups (id, organization_id, name, code, parent_item_group_id, is_active, created_at, updated_at)
                    VALUES (:id, :org, :name, :code, :parent, true, NOW(), NOW())"""),
            {"id": gid, "org": ORG_ID, "name": gname, "code": gcode, "parent": PRODUCTS_GROUP_ID},
        )
...
with open(CSV_PATH, "r") as f:
    ...
    for row in reader:
        ...
        if not group:
            print(f"SKIP (no group): {sku} — group '{row['item_group_name']}' not found")
            continue

        db.execute(
            text("""INSERT INTO items (id, organization_id, item_name, description, item_type, status, uom,
